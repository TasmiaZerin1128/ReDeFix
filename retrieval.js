...
async function retrieve(collectionDBName, cssProperties) {
  try {
    ...
    const rawCollection = await client.getOrCreateCollection({ name: collectionDBName });
    const { documents: texts, metadatas } = await rawCollection.get({
      include: ["documents", "metadatas"],
    });

    const docs = texts.map((txt, i) =>
      new Document({ pageContent: txt, metadata: metadatas[i] })
    );
    ...
    for(const question of questions) {
      ...
      const answers = docs.filter((d) => d.metadata.type === "answer" && d.metadata.question_id === questionId);
      question.answers = answers;
      const comments = docs.filter((d) => d.metadata.type === "comment" && d.metadata.question_id === questionId);
